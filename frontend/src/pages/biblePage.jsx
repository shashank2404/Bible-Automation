...
const SCENES = {
  books: (
    <svg viewBox="0 0 400 220" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%" }}>
      ...
      <circle cx="200" cy="38" r="3" fill="#fff9e6" opacity="0.95" />
      <circle cx="200" cy="38" r="8" fill="#f5a623" opacity="0.25" filter="url(#blur2)" />
  ...
  chapters: (
    <svg viewBox="0 0 400 180" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%" }}>
      <defs>
        <radialGradient id="sg" cx="50%" cy="60%" r="70%">
      ...
      <circle cx="200" cy="28" r="18" fill="#f5e6c8" opacity="0.9" />
      <circle cx="207" cy="24" r="14" fill="#0a1628" opacity="0.85" />
    ...
    <svg viewBox="0 0 400 160" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%" }}>
      <defs>
        <radialGradient id="rg" cx="50%" cy="0%" r="80%">
          <stop offset="0%" stopColor="#1a0e00" /><stop offset="100%" stopColor="#080500" />
        </radialGradient>
      ...
      {[116, 122, 128, 134, 140].map((y, i) => (
        <line key={i} x1={115 + i * 3} y1={y} x2={285 - i * 3} y2={y} stroke="#f5a623" strokeWidth="0.5" opacity="0.25" />
      ))}
...
const BOOK_THEMES = {
  Genesis: { accent: "#4fc3f7", label: "Creation", icon: "🌅" },
  Exodus: { accent: "#f5a623", label: "Liberation", icon: "🔥" },
  Psalms: { accent: "#a78bfa", label: "Praise", icon: "🎵" },
  Proverbs: { accent: "#34d399", label: "Wisdom", icon: "💎" },
  Isaiah: { accent: "#fb923c", label: "Prophecy", icon: "📯" },
  Daniel: { accent: "#60a5fa", label: "Vision", icon: "🦁" },
  Matthew: { accent: "#f472b6", label: "Gospel", icon: "✝️" },
  Mark: { accent: "#f59e0b", label: "Gospel", icon: "✝️" },
  Luke: { accent: "#6ee7b7", label: "Gospel", icon: "✝️" },
  John: { accent: "#93c5fd", label: "Gospel", icon: "✝️" },
  Revelation: { accent: "#f87171", label: "Apocalypse", icon: "🌟" },
  Jonah: { accent: "#38bdf8", label: "Mercy", icon: "🐋" },
  Ruth: { accent: "#fbbf24", label: "Loyalty", icon: "🌾" },
  "Song of Solomon": { accent: "#f9a8d4", label: "Love", icon: "🌹" },
  Job: { accent: "#94a3b8", label: "Suffering", icon: "⚖️" },
  Esther: { accent: "#c084fc", label: "Courage", icon: "👑" },
...
function ShareSheet({ isOpen, onClose, verseRef, verseText, accent }) {
  ...
  return (
    <>
      <style>{`
        .ss-overlay{position:fixed;inset:0;z-index:600;background:rgba(0,0,0,0.7);display:flex;align-items:flex-end;justify-content:center;animation:ssOvIn 0.18s ease;}
        @keyframes ssOvIn{from{opacity:0}to{opacity:1}}
        .ss-sheet{width:100%;max-width:500px;background:#0d0a02;border:1px solid rgba(245,166,35,0.22);border-bottom:none;border-radius:20px 20px 0 0;padding:0 0 env(safe-area-inset-bottom,0);animation:ssUp 0.28s cubic-bezier(0.32,0.72,0,1);overflow:hidden;}
        ...
        .ss-preview{margin-top:6px;padding:8px 12px;border-left:2px solid rgba(245,166,35,0.35);font-family:'EB Garamond',serif;font-size:13px;color:rgba(240,234,214,0.55);font-style:italic;line-height:1.5;background:rgba(245,166,35,0.04);border-radius:0 6px 6px 0;}
        .ss-apps{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;padding:18px 16px;}
        .ss-app-btn{display:flex;flex-direction:column;align-items:center;gap:7px;cursor:pointer;background:none;border:none;}
        .ss-app-icon{width:52px;height:52px;border-radius:14px;display:flex;align-items:center;justify-content:center;border:1px solid rgba(255,255,255,0.07);transition:transform 0.15s,filter 0.15s;}
        ...
        .ss-cancel{width:100%;padding:14px;border:none;background:rgba(245,166,35,0.07);border-top:1px solid rgba(245,166,35,0.1);color:rgba(240,234,214,0.45);font-family:'Cinzel',serif;font-size:12px;letter-spacing:0.1em;cursor:pointer;transition:background 0.15s;}
          ...
          <div className="ss-apps">
            {SHARE_APPS.map(app => (
              <button key={app.id} className="ss-app-btn"
                ...
                <div className="ss-app-icon" style={{ background: app.bg }}>{app.icon}</div>
...
function ContextMenu({ visible, x, y, verseNum, verseRef, theme, onAction, onClose }) {
  ...
  return (
    <>
      <style>{`
        .ctx-wrap{position:fixed;z-index:400;animation:ctxIn 0.17s ease;min-width:195px;background:rgba(10,6,0,0.97);border:1px solid rgba(245,166,35,0.28);border-radius:14px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.7);}
        @keyframes ctxIn{from{opacity:0;transform:scale(0.93) translateY(-4px)}to{opacity:1;transform:scale(1)}}
        .ctx-ref{padding:9px 14px 7px;font-family:'Cinzel',serif;font-size:10px;color:rgba(245,166,35,0.55);letter-spacing:0.15em;border-bottom:1px solid rgba(245,166,35,0.1);}
        .ctx-row{display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;border:none;background:none;width:100%;text-align:left;border-bottom:1px solid rgba(245,166,35,0.06);transition:background 0.12s;}
...
export default function BiblePage() {
  ...
  const [verseBookmarks, setVerseBookmarks]   = useState(new Set());
  const [chapterBookmarks, setChapterBookmarks] = useState(new Set());
  ...
  useEffect(() => {
    apiFetch("/bookmarks/verses").then(data => {
      if (Array.isArray(data))
        setVerseBookmarks(new Set(data.map(b => `${b.book} ${b.chapter}:${b.verse}`)));
    }).catch(() => {});

    apiFetch("/bookmarks/chapters").then(data => {
      if (Array.isArray(data))
        setChapterBookmarks(new Set(data.map(b => `${b.book} ${b.chapter}`)));
    }).catch(() => {});
  }, []);
  ...
  const handleCtxAction = async (action) => {
    ...
    if (action === "share") {
      ...
      setHighlightedVerses(prev => {
        ...
        n.has(vNum) ? n.delete(vNum) : n.add(vNum);
        return n;
      });
      ...
      const key = `${selectedBook.name} ${selectedChapter}:${vNum}`;
      try {
        const result = await apiFetch("/bookmarks/verses/toggle", {
          method: "POST",
          body: JSON.stringify({
        ...
        setVerseBookmarks(prev => {
          const n = new Set(prev);
          result.bookmarked ? n.add(key) : n.delete(key);
          return n;
        });
        toast(result.bookmarked ? "Verse bookmarked ✦" : "Bookmark removed ✦");
      } catch {
        toast("Failed to bookmark ✦");
      }
    }

    else if (action === "chapterbk") {
      const key = `${selectedBook.name} ${selectedChapter}`;
      try {
        const result = await apiFetch("/bookmarks/chapters/toggle", {
          method: "POST",
          body: JSON.stringify({ book: selectedBook.name, chapter: selectedChapter }),
        });
        setChapterBookmarks(prev => {
          const n = new Set(prev);
          result.bookmarked ? n.add(key) : n.delete(key);
          return n;
        });
        toast(result.bookmarked ? "Chapter bookmarked ✦" : "Chapter bookmark removed ✦");
      } catch {
        toast("Failed to bookmark chapter ✦");
      }
    }
    ...
    <>
      ...
      <div style={{
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            {view !== "books" && (
              <button className="back-btn" onClick={goBack} style={{
                ...
                fontSize: 13, fontFamily: "'Lato',sans-serif", transition: "all 0.2s",
              }}>← Back</button>
            )}
            <div style={{ flex: 1 }} />
            {view === "reading" && (
        ...
        <div ref={contentRef} style={{
          flex: 1, overflowY: "auto", padding: "16px", position: "relative",
              ...
              {["Old Testament", "New Testament"].map(section => {
                const books = filteredBooks.filter(b =>
                  section === "Old Testament"
                    ? BIBLE_BOOKS["Old Testament"].find(x => x.name === b.name)
                    : BIBLE_BOOKS["New Testament"].find(x => x.name === b.name)
                );
              ...
              {!loading && !error && verses.map((v, idx) => {
                ...
                return (
                  <div key={v.verse}
                    ...
                    <span style={{ fontFamily: "'Cinzel',serif", fontSize: 10, color: theme.accent, minWidth: 22, paddingTop: 4, opacity: isBookmarked ? 1 : 0.6, flexShrink: 0 }}>
